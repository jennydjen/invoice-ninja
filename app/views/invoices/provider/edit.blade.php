@extends('header')
<!--
Classe a revoir car copier/coller et tres peu de chose de modifier A revoir
-->
@section('head')
	@parent
...
@section('content')
	...
	@if ($invoice && $invoice->id)
		<ol class="breadcrumb">
			<li>{{ link_to(($entityType == ENTITY_QUOTE ? 'quotes' : 'invoices/provider'), trans('texts.' . ($entityType == ENTITY_QUOTE ? 'quotes' : 'invoices'))) }}</li>
			...
			@if ($entityType == ENTITY_INVOICE)
				...
				@if ($invoice && $invoice->recurring_invoice_id)
					<div class="pull-right" style="padding-top: 6px">
						Created by a {{ link_to('/invoices/provider/'.$invoice->recurring_invoice_id, 'recurring invoice') }}
					</div>
				@else 
					...
					<ul class="dropdown-menu">
						...
						@if ($invoice && $entityType == ENTITY_QUOTE)			
							...
							@if ($invoice->quote_invoice_id)
								<li><a href="{{ URL::to("invoices/provider/{$invoice->quote_invoice_id}/edit") }}">{{ trans("texts.view_invoice") }}</a></li>
							@else
